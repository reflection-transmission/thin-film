...
class CharacteristicMatrix(private val layer: Layer) {
    ...
    private fun phi(wavelength: Double): Double = 2 * PI * n(wavelength) * depth() / wavelength

    private fun phiPrime(wavelength: Double): Double = 2 * PI * k(wavelength) * depth() / wavelength
    ...
    private fun depth(): Double = layer.depth * 10.0.pow(-3)
